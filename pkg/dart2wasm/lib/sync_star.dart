...
class _YieldFinder extends StatementVisitor<void> {
  ...
  void recurse(Statement statement, {bool removeUnusedTargets = true}) {
    int yieldCountIn = yieldCount;
    int targetsIn = targets.length;
    statement.accept(this);
    if (removeUnusedTargets && yieldCount == yieldCountIn) {
  ...
  void visitTryFinally(TryFinally node) {
    ...
    recurse(node.body, removeUnusedTargets: false);
    addTarget(node, _StateTargetPlacement.Inner);
    recurse(node.finalizer, removeUnusedTargets: false);
    addTarget(node, _StateTargetPlacement.After);
  }
...
class ExceptionHandlerStack {
  ...
  /// in `innerTargets`.
  final List<ExceptionHandler> _catchBlocks = [];

  /// Current number of nested Wasm `try` blocks. Each `try` block will cover
  /// one list of handlers in [_catchBlocks].
  ...
  /// blocks for the `try` blocks generated by [generateWasmTryBlocks].
  void terminateWasmTryBlocks(SyncStarCodeGenerator codeGen) {
    // final numCatchBlocks = _catchBlocks.length;
...
abstract class ExceptionHandler {
  _StateTarget get target;
}

class Finalizer implements ExceptionHandler {
  final _StateTarget target;

  Finalizer(this.target);
}

class HandlerList implements ExceptionHandler {
  final List<Handler> handlers;

  _StateTarget get target => handlers.first.target;
