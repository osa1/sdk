...
class ExceptionHandlerStack {
  ...
  void generateWasmTryBlocks(w.Instructions b) {
    if (_catchBlocks.length == _catchBlockDepth) {
      return;
    }
  ...
  void terminateWasmTryBlocks(w.Instructions b) {
    final numCatchBlocks = _catchBlocks.length;
    assert(numCatchBlocks == _catchBlockDepth,
        '_catchBlocks.length = $numCatchBlocks, _catchBlockDepth = $_catchBlockDepth');
...
class Finalizer extends ExceptionHandler {
  final _StateTarget target;

  Finalizer(this.target);
}

class HandlerList extends ExceptionHandler {
